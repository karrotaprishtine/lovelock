...
function GameScreen({
  ...
  const opponentName = opponent?.name ?? "they";
  const answerStatus = session.answerTypingPlayerId === opponent?.id ? "typing..." : "thinking...";
  const boardTitle =
  ...
  return (
    <section className="screen screen--game">
      ...
      {session.phase === "question-pick" && (
        <Modal
          title={isWinner ? "pick one" : opponentName}
          subtitle={
            isWinner
              ? "25 max"
              : "picking..."
      ...
      {session.phase === "answering" && (
        <Modal
          title={isLoser ? "answer" : opponentName}
