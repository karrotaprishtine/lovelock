...
function App() {
  ...
  }, [appState.session?.phase, appState.session?.selectedQuestion?.id]);
  ...
  function nudgeLeaveButton(intensity = "soft") {
    const spreadX = intensity === "big" ? 34 : 18;
    const spreadY = intensity === "big" ? 18 : 10;
...
function GameScreen({
  ...
  return (
    <section className="screen screen--game">
      ...
      {session.phase === "question-pick" && (
        <Modal
          title={isWinner ? "pick one" : "thinking"}
          subtitle={
            isWinner
              ? "25 max"
              : "thinking..."
        ...
        >
          {isWinner ? (
            ...
            <div className="modal-placeholder">thinking...</div>
          )}
        </Modal>
      )}
      ...
      {session.phase === "answering" && (
        <Modal
          title={isLoser ? "answer" : "writing"}
          subtitle={session.selectedQuestion?.text}
        >
          {isLoser ? (
            ...
            <div className="modal-placeholder">writing...</div>
          )}
        </Modal>
      )}
